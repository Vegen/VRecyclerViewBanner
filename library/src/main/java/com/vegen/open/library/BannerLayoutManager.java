 ...
 * 主要参考：https://github.com/renjianan/RecyclerBanner 的实现方式进行修改
 */
...
public class BannerLayoutManager extends RecyclerView.LayoutManager {
     ...
     */
    public BannerLayoutManager(Context context, int showItemCount) {
        this(context, HORIZONTAL, false, showItemCount);
    }

    /**
     * @param orientation Layout orientation. Should be {@link #HORIZONTAL} or {@link #VERTICAL}
     */
    public BannerLayoutManager(Context context, int orientation, int showItemCount) {
        this(context, orientation, false, showItemCount);
    }

    public BannerLayoutManager(Context context, int orientation, boolean reverseLayout, int showItemCount) {
        setEnableBringCenterToFront(true);
        setMaxVisibleItemCount(showItemCount);
    ...
    private void layoutItems(RecyclerView.Recycler recycler) {
        ...
        for (int i = start; i < end; i++) {
            if (useMaxVisibleCount() || !removeCondition(getProperty(i) - mOffset)) {
                ...
                // params 3 : 是否是最后一个图或者只有一个图
                layoutScrap(scrap, targetOffset, itemCount == 1);
     ...
     * @param only         是否是最后一个或者是只有一个控件
     */
    private void layoutScrap(View scrap, float targetOffset, boolean only) {
